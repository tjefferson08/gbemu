...
(defn cartridge->header [f]
  (let [barr (.getBytes (slurp f))]
    {:entry-point (byte-slice barr 0x100 0x103)
     :nintendo-logo (byte-slice barr 0x104 0x133)}))

(defn load [f]
  (let [contents (vec (file->bytes f))
        header (cartridge->header f)]
    (swap! ctx assoc :rom-size (count contents))
    {:header header}))
...
(comment
  ...
  (let [barr (.getBytes "hello world")]
   (java.util.Arrays/copyOfRange barr 1 2)
   (java.util.Arrays/copyOfRange barr 3 5))

  (assoc {} :rom-size 100)

  (swap! ctx assoc :rom-size 100)

  (load "/Users/travis/LLD_gbemu/roms/01-special.gb")

  (file->bytes "/Users/travis/LLD_gbemu/roms/01-special.gb")
